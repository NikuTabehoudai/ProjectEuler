using System;
using System.Collections.Generic;
using System.Text;

namespace ProjectEuler
{
    class LargestProduct
    {

        public LargestProduct()
        {
            string input = "";
            int target = 13;
            long output = 0;

            Char[] inputA = input.ToCharArray();

            List<long> list = new List<long>();

            var templist = new List<long>();

            foreach (var item in inputA)
            {
                list.Add(Int16.Parse(item.ToString()));
            }

            while (list.Count >= target)
            {

                var tempOutput = Product(list, target);
                
                if (tempOutput > output)
                {
                    output = tempOutput;

                }

                list.RemoveAt(0);
            }

            templist.Sort();
            Console.WriteLine("Largest product of " + target + " adjacent numbers in input");
            Console.WriteLine(output);


        }


        public long Product(List<long> iList, int target)
        {
            long product = 1;

            for (int i = 0; i < target; i++)
            {
                product = product * iList[i];

            }

            
            return product;

        }
    }
}
